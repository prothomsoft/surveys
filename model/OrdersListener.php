...
class model_OrdersListener extends MachII_framework_Listener
{
		...
		while ($aRow = mysql_fetch_array($rResult))	{
			...
			for ($i=0; $i<count($aColumns); $i++) {
				...
				if ($aColumns[$i] == "Amount"){
					$responseJSON .= '"'.$aRow[ $aColumns[$i] ].' NOK",';
    ...
    function createOrder(&$event) {
    	...
    	if($event->getArg("OrderId") == "" && $event->getArg("email") != "") {
			...
			$shipPrice = $objAppSession->getSession("shipPrice");
			...
			$cartWeightTotal = 0;
	      	foreach($event->getArg('arrProduct') as $objProduct) {
				...
				$objOrdersProductBean->setPurchaseWeight($objProduct->getWeight());
			...
			$messageContentTable .= "<table width=\"550\" cellspacing=\"0\" cellpadding=\"10\" border=\"0\">";
			$messageContentTable .= "<tr>";
			...
			$messageContentTable .= "<tr>";
	  		...
	  		$messageContentTable .= "</td>";
	  		$messageContentTable .= "<td width=\"100\" align=\"center\">";
	  		$messageContentTable .= $shipPrice." NOK";
	  		$messageContentTable .= "</td>";
			$messageContentTable .= "</tr>";
			$messageContentTable .= "<tr>";
	  		$messageContentTable .= "<td width=\"550\" align=\"right\">";
    	...
    	if($email != "") {
	    	...
	    	$toEmail2 = "musikkglede@hotmail.com";
			$mail = new Rmail();
	    	$mail->setFrom($fromEmail2); 
	    	$mail->setSubject($subject2);
		    $mail->setHTML($messageContent2);
	    	...
	    	$toEmail2 = "musikkglede@musikkglede.no";
			$mail = new Rmail();
	    	$mail->setFrom($fromEmail2); 
	    	$mail->setSubject($subject2);
		    $mail->setHTML($messageContent2);
    ...
    function sendEmail(&$event) {
			...
			$mail = new Rmail();
	    	$mail->setFrom("musikkglede@musikkglede.no"); 
