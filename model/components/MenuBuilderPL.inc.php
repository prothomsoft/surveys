...
class MenuBuilderPL
{
	...
	function get_menu_items()
	{
		...
		$sql = 'SELECT CmsCategoryId, FatherId, Name, Url, ListOrder FROM CmsCategory WHERE FatherId != 66 AND CmsCategoryId != 113 ORDER BY FatherId, ListOrder;';
	...
	function get_menu_html( $root_id = 0 )
	{
		...
		while ( $loop && ( ( $option = each( $children[$parent] ) ) || ( $parent > $root_id ) ) )
		{
			...
			elseif ( !empty( $children[$option['value']['CmsCategoryId']] ) )
			{
				...
				$this->html[] = sprintf(
					'%1$s<li class="dropdown"><a href="%2$s" class="dropdown-toggle" data-toggle="dropdown">%3$s <span class="caret"></span></a>',
					$tab,   // %1$s = tabulation
				...
				if($option['value']['Name'] != "Referanser") {
					$this->html[] = sprintf(
