...
class TopicDao{
   ...
   public function create($objTopicBean){
      $DB = new DB();
      $DB->connect();
      $query = "INSERT INTO Topic(Question,OpenQuestion,CreateDate,Status,TopicOrder) ";
      $query.= "VALUES('".$objTopicBean->getQuestion()."','".$objTopicBean->getOpenQuestion()."','".$objTopicBean->getCreateDate()."','".$objTopicBean->getStatus()."','".$objTopicBean->getTopicOrder()."') ";
      $DB->query($query);
      return $DB->getLast();
   }
